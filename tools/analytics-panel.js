...
(function () {
  ...
  function anLoad() {
    // cms.html runs Firebase inside a module, so its db/query bindings are not
    // on window.MB.firebase like the public pages. It exposes them as MB.cmsFs.
    var C = window.MB && window.MB.cmsFs;
    if (!C || !C.db) { elStatus.textContent = 'Not connected.'; return; }
    ...
    // traffic
    var qStats = C.query(C.collection(C.db, STATS_COLL),
                         C.where('date', '>=', from), C.orderBy('date'));
    ...
    var ts = C.Timestamp.fromDate(fromDate);

    Promise.all([
      C.getDocs(qStats),
      C.getDocs(C.collection(C.db, LIST_COLL)),
      C.getDocs(C.collection(C.db, 'listings'))
    ].concat(leadCols.map(function (c) {
      return C.getDocs(C.query(C.collection(C.db, c), C.where('createdAt', '>=', ts)))
        .catch(function () { return { size: 0, forEach: function () {} }; });
    })))
    .then(function (res) {
      var statsSnap = res[0], listSnap = res[1], listingsSnap = res[2];
      var leadCounts = {};
      leadCols.forEach(function (c, i) { leadCounts[c] = res[3 + i].size || 0; });

      // id -> street address, so the table reads as places not document ids
      var addrById = {};
      listingsSnap.forEach(function (d) {
        var v = d.data() || {};
        if (v.address) addrById[d.id] = v.address.street + ', ' + v.address.city;
      });
